...
class BetterPlayerController {
  ...
  Future _setupAsmsDataSource(BetterPlayerDataSource source) async {
    ...
    if (data != null) {
      ...
      if (_betterPlayerDataSource?.useAsmsTracks == true) {
        ...
        setTrackWithMaxBitrate(_betterPlayerAsmsTracks);
      }
  ...
  Future _setupDataSource(BetterPlayerDataSource betterPlayerDataSource) async {
    switch (betterPlayerDataSource.type) {
      case BetterPlayerDataSourceType.network:
        await videoPlayerController?.setNetworkDataSource(betterPlayerDataSource.url,
            headers: _getHeaders(),
  ...
  void _displaySkipIntroButton(VideoPlayerValue currentVideoPlayerValue) {
    if (betterPlayerSkipIntroConfiguration != null) {
      if (currentVideoPlayerValue.position.inMilliseconds >=
              betterPlayerSkipIntroConfiguration!.skipIntroDetails.skipIntroShowMillis &&
          currentVideoPlayerValue.position.inMilliseconds <
              betterPlayerSkipIntroConfiguration!.skipIntroDetails.skipIntroHideMillis) {
        showSkipIntroButton();
      } else if (currentVideoPlayerValue.position.inMilliseconds >
          betterPlayerSkipIntroConfiguration!.skipIntroDetails.skipIntroHideMillis) {
        hideSkipIntroButton();
